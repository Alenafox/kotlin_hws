...
def neighbors2(B, y, x): #поиск соседей слева и сверху
    left = y, x-1
    top = y - 1, x
    if not check(B, *left):
        left = None
...
def find(label, linked): #поиск родительской метки в графе
    j = label
    while linked[j] != 0:
        j = linked[j]
    return j

def union(label1, label2, linked): #добавление метки в граф
...
def two_pass_labeling(B):
    ...
    for row in range(B.shape[0]):
        for col in range(B.shape[1]):
            if B[row, col] != 0: #если замечен кусок фигуры
                n = neighbors2(B, row, col) #смотрим соседей
                if not exists(n):
                    m = label
                    label += 1
                else:
